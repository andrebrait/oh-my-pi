...
- `extensibility/skills.ts` then:
  ...
  - when same-named skills have differing bodies, every variant receives a `<namespace>/<name>` prefix so all colliding variants are explicitly disambiguated, and emits collision warnings naming the paths. The namespace is the directory owning the skill's `skills/` tree — the plugin name for marketplace and package installs — or the skill root's directory name, falling back to the provider id for dotted homes such as `~/.claude/skills`. A namespaced slot that is itself already taken by a differing skill gets a `~2`, `~3`, … suffix; nothing is dropped without a warning.
...
- Custom-directory skills are merged after provider skills and override same-named default-path provider skills; the displaced skill is re-admitted under its namespaced name (suffixed if that slot is taken) when the bodies differ. Among custom directories the same content/namespace rules apply.
